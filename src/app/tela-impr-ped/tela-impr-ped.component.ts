...
@Component({
  ...
  templateUrl: './tela-impr-ped.component.html',
  styleUrl: './tela-impr-ped.component.css'
})


export class TelaImprPedComponent {
  ...
  @ViewChild('ttDadosPed') GridConclusao!: PoTableComponent;
  ...
  public formRel = this.formImport.group({
    cPedIni: ['0'],
    cPedFim: ['999999999'],
    cUser: ['']
  });
  ...
  ngOnInit(): void {
    ...
    //Colunas do grid
    this.colunasPed = this.srvTotvs.obterColunasPed()
  ...
  loadMoreItens(){
    ...
    this.loadTela = true
    //this.desabilitaForm()
    let paramsTela: any = { items: this.formRel.value, page: this.page, pageSize: this.pageSize }
    
    this.srvTotvs.ObterDadosRelPed(paramsTela).subscribe({
      next: (response: any) => {
        ...
        this.lista.sort(this.srvTotvs.ordenarCampos(['codEstabel']))
  ...
  ChamaObterDadosRelPed(){
    
    const inicial = Number(this.formRel.controls['cPedIni'].value)
    const final   = Number(this.formRel.controls['cPedFim'].value)

    if (this.formRel.controls['cUser'].value && (final - inicial > 100)){
      this.srvNotification.error("Faixa de pedidos deve ser menor quando for informado Usuário de Implantação (Máximo 100) ")
      return
    }
    else {
  ...
  public obterarq() {
    ...
    let paramsnrPed: any = {inrPed: this.itemSelecionado.nrPedido}

    this.srvTotvs.ObterImprimePd(paramsnrPed).subscribe({
      next: (item: any) => {
        //Arquivo Gerado
        let paramsArq: any = { nrProcess: this.itemSelecionado.nrPedido, situacao: 'IPED' };
        this.srvTotvs.ObterArquivo(paramsArq).subscribe({
          ...
          error: (e: any) => {
            //mensagem pro usuario
            //this.srvNotification.error("Erro ao chamar ExecEntradas.:" + e)
          }
  ...
  ImprimirSelecionado() {
    ...
    registrosSelecionados.forEach((item) => { registrosComSeparador += item['nrPedido'] + ';' })
    ...
    let paramsnrPed: any = {inrPed: registrosComSeparador} //this.itemSelecionado.nrPedido}

    this.srvTotvs.ObterImprimePd(paramsnrPed).subscribe({
      next: (item: any) => {
        //Arquivo Gerado
        registrosSelecionados.forEach((item) => {  
  ...
  public onExportarExcel() {
    let titulo = "Relatório Impressão de Pedidos" //this.tituloTela.split(':')[0]
    let subTitulo = "Pedidos Listados" //this.tituloTela.split(':')[1]
    this.loadExcel = true

    //let valorForm: any = { valorForm: this.form.value }

    this.srvExcel.exportarParaExcel('Pedidos Listados: ' + titulo.toUpperCase(),
